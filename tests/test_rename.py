...
def test_file(tmp_path, rename_input_file):
    ...
    with patch('sys.argv', ['pyrecli', 'rename', str(rename_input_file), 'MY_CONSTANT', 'RENAMED_CONSTANT', '--output_path', str(output_file)]):
        result = main()
        assert result == 0
...
def test_stdout(rename_input_file):
    with patch('sys.argv', ['pyrecli', 'rename', str(rename_input_file), 'MY_CONSTANT', 'RENAMED_CONSTANT', '--output_path', '-']):
        result = main()
        assert result == 0


def test_local(rename_input_file):
    with patch('sys.argv', ['pyrecli', 'rename', str(rename_input_file), 'i', 'j', '-s', 'line', '--output_path', '-']):
